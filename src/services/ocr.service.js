...
class OCRService {
  constructor() {
    ...
    this.lastScreenshot = null; // Store last screenshot for AI context
  }
  ...
  /**
   * Capture screenshot and return base64 image data for AI vision
   * This method captures the screen and stores it for AI context
   */
  async captureScreenshotForAI() {
    const startTime = Date.now();
    
    try {
      logger.info('Capturing screenshot for AI vision analysis');
      
      const screenshot = await this.captureScreenshot();
      const base64Image = screenshot.image.toPNG().toString('base64');
      
      // Store the screenshot for context
      this.lastScreenshot = {
        base64: base64Image,
        mimeType: 'image/png',
      ...
      logger.info('Screenshot captured for AI', {
        imageSize: base64Image.length,
        dimensions: screenshot.metadata.dimensions,
        processingTime: Date.now() - startTime
      });
      ...
      logger.error('Failed to capture screenshot for AI', { error: error.message });
      throw error;
    }
  }

  /**
   * Get the last captured screenshot for AI context
   */
  getLastScreenshot() {
  ...
  /**
   * Clear the stored screenshot context
   */
  clearScreenshotContext() {
    this.lastScreenshot = null;
    logger.debug('Screenshot context cleared');
  }
